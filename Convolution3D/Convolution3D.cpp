...
public:
  ...
  Convolution3D(PClip _child, short _matrix, short _luma_Treshold, short _chroma_Treshold,
    ...
    double _temporal_influence, int _debug, bool _use_smp, bool _use_simd, IScriptEnvironment* env)
    ...
    temporal_influence(_temporal_influence), debug(_debug), use_smp(_use_smp), use_simd(_use_simd)
  {
    ...
    smp = !use_simd && use_smp && (siSysInfo.dwNumberOfProcessors > 1);
...
AVSValue __cdecl Create_Convolution3D(AVSValue args, void* user_data, IScriptEnvironment* env) {
  return new Convolution3D(args[0].AsClip(), args[1].AsInt(0),					// Matrix choice
    ...
    args[8].AsBool(false), // smp = false by default
    args[9].AsBool(false), // simd = false by default
    env);				// debug
}
...
AVSValue __cdecl Create_Convolution3D_Pre(AVSValue args, void* user_data, IScriptEnvironment* env) {
  ...
  {
    return new Convolution3D(args[0].AsClip(), 0, 3, 4, 3, 4, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else if (!stricmp(myString, "movieLQ"))
  {
    return new Convolution3D(args[0].AsClip(), 0, 6, 10, 6, 8, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else if (!stricmp(myString, "animeHQ"))
  {
    return new Convolution3D(args[0].AsClip(), 0, 6, 12, 6, 8, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else if (!stricmp(myString, "animeLQ"))
  {
    return new Convolution3D(args[0].AsClip(), 1, 8, 16, 8, 8, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else if (!stricmp(myString, "animeBQ"))
  {
    return new Convolution3D(args[0].AsClip(), 1, 12, 22, 8, 8, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else if (!stricmp(myString, "vhsBQ"))
  {
    return new Convolution3D(args[0].AsClip(), 0, 16, 48, 10, 32, 4, 0, args[2].AsBool(false), args[3].AsBool(false), env);
  }
  else
  {
    env->ThrowError("Correct preset values are : movie[HQ, LQ], anime[HQ, LQ, BQ] or vhs[BQ]");
  }
  return new Convolution3D(args[0].AsClip(), 1, 6, 12, 6, 8, 2.8, 0, args[2].AsBool(false), args[3].AsBool(false), env);
}
